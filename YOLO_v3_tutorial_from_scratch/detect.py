...
File Created: Monday, 11th June 2018 11:00:34 am
Author: xiaofeng (sxf1052566766@163.com)
-----
Last Modified: Thursday, 30th August 2018 1:02:26 pm
...
def arg_parse():
    ...
    parser.add_argument(
        "--det", dest='det', help="Image / Directory to store detections to",
    ...
    parser.add_argument("--weights", dest='weightsfile', help="weightsfile",
                        default="./saved_model/yolov3.weights", type=str)
...
assert os.path.exists(args.weightsfile), 'Darknet weight file not exist'
...
model = Darknet(args.cfgfile, False)
...
print('Write parameters to json')
layer_dict = OrderedDict()
for layer_name, parameters in model.named_parameters():
    layer_dict[layer_name] = {}
    layer_dict[layer_name]['size'] = parameters.size()
    # layer_dict[layer_name]['value'] = parameters.value()
with open(opt.model_json_path, 'w') as layer:
    json.dump(layer_dict, layer, ensure_ascii=False)
print('Json file was done')

model.net_info["height"] = args.reso
inp_dim = int(model.net_info["height"])
# 整个网络机构下采样之后上采样，整个下采样为32
assert inp_dim % 32 == 0
assert inp_dim > 32
...
# The image format read by opencv is BGR, but the format pytorch needed is RGB
loaded_ims = [cv2.imread(x) for x in imlist]

im_batches = list(map(prep_image, loaded_ims, [inp_dim for x in range(len(imlist))]))
# Img size list(width,height)
...
for i, batch in enumerate(im_batches):
    ...
    if CUDA:
        """ 
        orch.cuda.synchronize确保CUDA内核与CPU同步。否则，CUDA内核会在GPU作业排队后立即将控制返回给CPU，
