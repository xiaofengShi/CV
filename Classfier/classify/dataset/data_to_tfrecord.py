...
print('Please confirm the resize shape is :',
      cfg.IMAGE_HEIGHT, cfg.IMAGE_WIDTH, cfg.IMAGE_CHANNEL)
...
def createTFRecord(dataset_dir, tfrecorder_dir, mapfile, shuffling, train_or_test):
    ...
    with tf.python_io.TFRecordWriter(tfrecorder_dir) as tfrecorder_writer:
        for index, name in enumerate(classes):
            ...
            # 直接得到文件夹下的图片的路径
            img_name_list = GetFileFromThisRootDir(
                [class_path], ext=cfg.EXTENSION)
            if shuffling:
            ...
            print('\n  %s / %s___Completed the filename: %s' %
                  (files_count, how_many_files, name))
            files_count += 1
...
def read_and_decode(filename):
    ...
    # reshape为128*128*3通道图片
    img = tf.reshape(
        img, [cfg.IMAGE_HEIGHT, cfg.IMAGE_WIDTH, cfg.IMAGE_CHANNEL])
...
def run_dataset_tfrecord(shuffling=False, is_training=True):
    ...
    if is_training:
        ...
        tfrecord, txt = get_output_filename(
            output_tfrecord_txt, cfg.Project_NAME, cfg.TRAIN)
        batch_size = cfg.BATCH_SIZE
    if not is_training:
        dataset = cfg.TEST_DATASET
        tfrecord, txt = get_output_filename(
            output_tfrecord_txt, cfg.Project_NAME, cfg.TEST)
