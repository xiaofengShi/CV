...
class Detector(object):
    ...
    def __init__(self, net, weight_file):
        ...
        self.image_size_channel = cfg.IMAGE_CHANNEL

        self.config = tf.ConfigProto(device_count={"CPU": 1},  # limit to num_cpu_core CPU usage
                                     inter_op_parallelism_threads=1,
                                     intra_op_parallelism_threads=3,  # limit the threads
                                     log_device_placement=False
                                     )

        self.sess = tf.Session(self.config)
        ...
        self.images, self.labels = run_dataset_tfrecord(is_training=False, shuffling=False)

    def detect(self, img):
        img_h, img_w, _ = img.shape
        inputs = cv2.resize(img, (self.image_size_height, self.image_size_width))
        inputs = cv2.cvtColor(inputs, cv2.COLOR_BGR2RGB).astype(np.float32)
        inputs = (inputs / 255.0) * 1.0 - 0.5
        inputs = np.reshape(inputs, (1, self.image_size_height, self.image_size_width, 3))
