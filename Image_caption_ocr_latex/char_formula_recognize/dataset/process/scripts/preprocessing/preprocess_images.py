...
def main_parallel(l):
    ...
    if not status:
        logging.info(
            '%s is blank, crop a white image of default size!' % filename)
    status = pad_group_image(output_filename, output_filename, pad_size,
                             buckets)
    if not status:
        logging.info(
            '%s (after cropping and padding) is larger than the largest provided bucket size, left unchanged!'
            % filename)
    status = downsample_image(output_filename, output_filename,
                              downsample_ratio)
...
def main(args):
    ...
    # print('crop_blank_default_size, pad_size, buckets, downsample_ratio:',
