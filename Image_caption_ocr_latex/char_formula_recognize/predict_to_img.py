...
ckpt_path = '/Users/xiaofeng/Desktop/model_generate_enahce/'
...
net_fun = tflib.network.net_fun(net_name='resnet_v2_50', num_classes=None)
...
properties = np.load('/Users/xiaofeng/Desktop/properties_generate_enhace.npy').tolist()
...
def predict_img_latex(file, target, Flage=False, calculate_falge=False):
...
def main(flage=True, draw_flag=False):
    if flage:
        if os.path.exists(cfg.IMG_DATA_PATH):
            file_list = GetFileFromThisRootDir([cfg.IMG_DATA_PATH], ['png', 'jpg'])
...
if __name__ == '__main__':
    ...
    #     os.mkdir(cfg.PREDICT_PATH_LOG)
    main(flage=True, draw_flag=False)
