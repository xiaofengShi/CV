...
def Embedding(name, n_symbols, output_dim, indices):
    ...
    with tf.name_scope(name) as scope:
        emb = tflib.param(name,
                          initializer(
                              name='Normal',
                              shape=[n_symbols, output_dim],
                              std=1.0 / np.sqrt(n_symbols)))
...
class im2latexAttentionCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, _input, state, scope=None):
        ...
        output_t = tf.tanh(
            tflib.ops.Linear(self._name + '.output_t',tf.concat(axis=1, values=[h_t, z_t]),self._D + self._n_hid,self._n_hid,bias=False,activation='tanh'))
