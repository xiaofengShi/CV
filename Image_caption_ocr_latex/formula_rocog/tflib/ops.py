...
def Embedding(name, n_symbols, output_dim, indices):
    ...
    with tf.name_scope(name) as scope:
        emb = tflib.param(name, initializer(name='Normal', shape=[
                          n_symbols, output_dim], std=1.0 / np.sqrt(n_symbols)))
...
def conv2d(name, input, kernel, stride, depth, num_filters, init='GlorotUniform', pad='SAME', bias=True,
    ...
    """
    Performs 2D convolution on input in NHWC data format
...
def max_pool(name, l_input, k, s):
    """
    Max pooling operation with kernel size k and stride s on input with nhwc data format
...
class LSTMCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        gates = tflib.ops.Linear(
            self._name + '.Gates', tf.concat(axis=1, values=[inputs, h_tm1]),
            self._n_in + self._n_hid, 4 * self._n_hid, activation='sigmoid')
...
def BiLSTM(name, inputs, n_in, n_hid, h0_1=None, h0_2=None):
    ...
    outputs = tf.nn.bidirectional_dynamic_rnn(
        cell_fw, cell_bw, inputs, sequence_length=seq_len, initial_state_fw=h0_1, initial_state_bw=h0_2,
        swap_memory=True)
...
class im2latexAttentionCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, _input, state, scope=None):
        ...
        gates = tflib.ops.Linear(
            self._name + '.Gates', tf.concat(axis=1, values=[_input, output_tm1]),
            self._n_in + self._n_hid, 4 * self._n_hid, activation='sigmoid')
        ...
        target_t = tf.expand_dims(tflib.ops.Linear(self._name + '.target_t',    h_t,
                                                   self._n_hid,    self._n_hid,    bias=False), 2)
...
def im2latexAttention(name, inputs, ctx, input_dim, ENC_DIM, DEC_DIM, D, H, W):
    ...
    # V = tf.transpose(ctx, [0, 2, 3, 1])  # NHWC --->>>>(N, H, W, D)
    # V = ctx
    V_cap = []
    ...
    h0_i_1 = tf.tile(
        tflib.param(name + '.Enc_.init.h0_1',
                    np.zeros((1, H, 2 * ENC_DIM)).astype('float32')), [batch_size, 1, 1])
    ...
    def fn(prev_out, i):
        # for i in xrange(H):
        # name, inputs, n_in, n_hid, h0_1 = None, h0_2 = None
    ...
    h0_dec = tf.tile(tflib.param(name + '.Decoder.init.h0',
                                 np.zeros((1, 3 * DEC_DIM)).astype('float32')), [batch_size, 1])
    cell = tflib.ops.im2latexAttentionCell(name=name + '.AttentionCell',
                                           n_in=input_dim, n_hid=DEC_DIM, L=H * W, D=2 * ENC_DIM, ctx=V_t)
    seq_len = tf.tile(tf.expand_dims(tf.shape(inputs)[1], 0), [batch_size])
...
class FreeRunIm2LatexAttentionCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, _input, state, scope=None):
        h_tm1, c_tm1, output_tm1 = tf.split(
            axis=1, num_or_size_splits=3, value=state[:, :3 * self._n_hid])
        _input = tf.argmax(state[:, 3 * self._n_hid:], axis=1)
        _input = tflib.ops.Embedding('Embedding', self._n_out, self._n_in,
                                     _input)
        ...
        # target_t = tf.expand_dims(h_t,2) # (B, HID, 1)
        a_t = tf.nn.softmax(
            tf.matmul(self._ctx, target_t)[:, :, 0],
            name='a_t')  # (B, H*W, D) * (B, D, 1)
        a_t = tf.expand_dims(a_t, 1)  # (B, 1, H*W)
        z_t = tf.matmul(a_t, self._ctx)[:, 0]
        # a_t = tf.expand_dims(a_t,2)
        # z_t = tf.reduce_sum(a_t*self._ctx,1)
        ...
        logits = tf.nn.softmax(
            tflib.ops.Linear('MLP.1', output_t, self._n_hid, self._n_out))
        new_state = tf.concat(axis=1, values=[h_t, c_t, output_t, logits])

        return logits, new_state


def FreeRunIm2LatexAttention(name, ctx, input_dim, output_dim, ENC_DIM,
                             DEC_DIM, D, H, W):
    """
    Function that encodes the feature grid extracted from CNN using BiLSTM encoder
    and decodes target sequences using an attentional decoder mechanism
    ...
    V_t = tf.reshape(
        ...
        [batch_size, -1, ENC_DIM * 2])  # (B, L, ENC_DIM)
    ...
    h0_dec = tf.concat(
        ...
        values=[
            tf.tile(
                ...
                [batch_size, 1]),
            tf.reshape(tf.one_hot(500, output_dim), (batch_size, output_dim))
        ])

    inputs = tf.zeros((batch_size, 160, 100))

    cell = tflib.ops.FreeRunIm2LatexAttentionCell(
        name + '.AttentionCell', input_dim, output_dim, DEC_DIM, H * W,
        2 * ENC_DIM, V_t)
