...
# -*- coding: utf-8 -*-
# _Author_: xiaofeng
# Date: 2018-04-01 11:38:52
# Last Modified by:   xiaofeng
# Last Modified time: 2018-04-01 11:38:52
...
# 0-large data set(for gpu), 1-small data set(for cpu debug)
SMALL_DATA_SET = 0
MODEL_SAVED = cfg.MODEL_SAVED_PATH
saved_models_name = os.path.join(MODEL_SAVED, 'im2latex.ckpt')
summaries_dir = cfg.SUMMARY_PATH
idx_to_vocab = None
vocab_to_idx = None
...
def load_data():
    ...
    train = open(cfg.TRAIN_PATH).read().split('\n')[:-1]
    val = open(cfg.VALIDATE_PATH).read().split('\n')[:-1]
    test = open(cfg.TEST_PATH).read().split('\n')[:-1]
    print('length  train, val, test', len(train), len(val), len(test))
    # if SMALL_DATA_SET:
    ...
    def import_images(datum):
        datum = datum.split(' ')
        img = np.array(Image.open(cfg.IMAGE_PATH + datum[0]).convert('L'))
...
def batchify(data, batch_size):
    ...
    for size in res:
        ...
        for i in range(0, len(group), batch_size):
            images = list(
                map(lambda x: np.expand_dims(np.expand_dims(x[0], 0), 3),
            ...
            too_big = [(160, 400), (100, 500), (100, 360), (60, 360),
                       (50, 400), (100, 800), (200, 500), (800, 800),
                       (100, 600)]  # these are only for the test set
...
def init_cnn(inp):
    ...
    h_pool2 = tf.nn.max_pool(
        h_bn2, ksize=[1, 1, 2, 1], strides=[1, 1, 2, 1], padding='SAME')
    ...
    h_pool3 = tf.nn.max_pool(
        h_conv3, ksize=[1, 2, 1, 1], strides=[1, 2, 1, 1], padding='SAME')
    ...
    h_pool5 = tf.nn.max_pool(
        h_conv5, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
    ...
    h_pool6 = tf.nn.max_pool(
        h_pad6, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
    return h_pool6
...
def build_model(inp, batch_size, num_rows, num_columns, dec_seq_len):
    ...
    def fn(inp):
        ...
        return tf.concat(values=output, axis=2)
    ...
    dec_init_state = tf.nn.rnn_cell.LSTMStateTuple(
        tf.truncated_normal(dec_init_shape), tf.truncated_normal(dec_init_shape))

    # init_words = np.zeros([batch_size, 1, vocab_size])

    decoder_output = decoder.embedding_attention_decoder(
        dec_init_state, tf.reshape(encoder_output, [batch_size, -1, 2 * enc_lstm_dim]),
        dec_lstm_cell, vocab_size, dec_seq_len, batch_size, embedding_size, feed_previous=True)

    return (encoder_output, decoder_output)


batch_size = cfg.BATCH_SIZE
epochs = cfg.EPOCH_NUMS
...
_, (output, state) = build_model(inp, batch_size, num_rows, num_columns,
                                 num_words)
cross_entropy = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(
    logits=output, labels=true_labels))
...
def run_train():
    ...
    with tf.Session() as sess:
        ...
        if not tf.gfile.Exists(MODEL_SAVED):
            tf.gfile.MakeDirs(MODEL_SAVED)
        sess.run(initop)
        merged_op = tf.summary.merge_all()
        writer = tf.summary.FileWriter(summaries_dir, sess.graph)
        saver = tf.train.Saver(
            max_to_keep=1, keep_checkpoint_every_n_hours=0.5)
        if os.path.exists(os.path.join(MODEL_SAVED, 'checkpoint')):
            print('Restoring weights from: ', MODEL_SAVED)
        ...
        for i in range(epochs):
            ...
            for j in range(len(train)):
                try:
                    ...
                    train_accuracy_value = accuracy.eval(
                        feed_dict={
                            inp: images,
                            true_labels: labels,
                    ...
                    if global_step % cfg.DISPLAY_NUMS == 0:
                        print(
                            'step/epoch/epoch_total %d/%d/%d, training accuracy %g'
                            % (global_step, i, epochs, train_accuracy_value))
                    if global_step % cfg.SAVED_NUMS == 0:
                        summary, loss, _ = sess.run(
                            [merged_op, cross_entropy, train_step],
                            feed_dict={
                                learning_rate: lr,
                                inp: images,
                                true_labels: labels,
                        ...
                        print('saver and summary are saving to ',
                              saved_models_name, '\n', summaries_dir)
                        saver.save(sess, saved_models_name, global_step)
                        writer.add_summary(summary, global_step)
                except Exception as e:
                    with open('log.txt', 'a') as f:
                        error_time = datetime.datetime.now().strftime(
                            '%Y-%m-%d %H:%M:%S')
                        write = error_time + '  ' + 'Reason:' + e + '\n'
                        f.writelines(write)
                    f.close()
                ...
                for j in range(len(val)):
                    try:
                        images, labels = val[j]
                        val_accuracy_value_acc = accuracy.eval(
                            feed_dict={
                                inp: images,
                                true_labels: labels,
                                num_rows: images.shape[1],
                        ...
                        accs.append(val_accuracy_value_acc)

                        if j % cfg.DISPLAY_NUMS == 0:
                            print('{}/{}-val accuracy{}'.format(
                                j, len(val), val_accuracy_value_acc))
                    except Exception as e:
                        with open('log.txt', 'a') as f:
                            error_time = datetime.datetime.now().strftime(
                                '%Y-%m-%d %H:%M:%S')
                ...
                print("val accuracy %g" % val_accuracy_value)
...
def run_sample():
    saver = tf.train.Saver()
    with tf.Session() as sess:
        ckpt = tf.train.latest_checkpoint(MODEL_SAVED)
        print(ckpt)
