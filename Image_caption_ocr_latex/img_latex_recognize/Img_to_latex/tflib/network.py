...
def vgg16(X, num_feats=64):
    ...
    X = tf.nn.relu(tflib.ops.conv2d('conv4_1', X, 3, 1, 4 * num_feats,
...
def im2latex_cnn(X, num_feats, bn, train_mode=True):
    ...
    # X = tf.nn.relu(tflib.ops.conv2d('conv1', X, 3, 1, 3, num_filters=num_feats, pad='SAME', bias=False))
...
def net_fun(net_name,
    ...
    def network_fn(inputs, **kwargs):
        ...
        with slim.arg_scope(arg_scope):
            return func(inputs, num_classes, is_training=is_training, global_pool=global_pool, **kwargs)
    ...
    if net_name == 'vgg_16':
        ...
        with slim.arg_scope(resnet_arg_scope()):
            net, end_points = resnet_v2.resnet_v2_50(
                inputs=inputs, num_classes=None, is_training=train_model, global_pool=False)
