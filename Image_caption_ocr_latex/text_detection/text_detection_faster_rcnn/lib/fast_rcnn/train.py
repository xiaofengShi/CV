...
class SolverWrapper(object):
    def __init__(self, sess, network, imdb, roidb, output_dir, logdir,
    ...
    def snapshot(self, sess, iter):
        ...
        if cfg.TRAIN.BBOX_REG and 'bbox_pred' in net.layers and cfg.TRAIN.BBOX_NORMALIZE_TARGETS:
            ...
            sess.run(weights.assign(orig_0 * np.tile(self.bbox_stds,
                                                     (weights_shape[0], 1))))
    ...
    def train_model(self, sess, max_iters, restore=False):
        ...
        total_loss, model_loss, rpn_cross_entropy, rpn_loss_box, cross_entropy, loss_box = self.net.build_loss(
            ohem=cfg.TRAIN.OHEM)
        # scalar summary
        tf.summary.scalar('rpn_reg_loss', rpn_loss_box)
        tf.summary.scalar('rpn_cls_loss', rpn_cross_entropy)
        tf.summary.scalar('reg_loss', loss_box)
        ...
        timer = Timer()
        print('restore_iter, max_iters', restore_iter, max_iters)
        for iter in range(restore_iter, max_iters):
            timer.tic()
            # learning rate
            if iter != 0 and iter % cfg.TRAIN.STEPSIZE == 0:
                tiems = int(iter/cfg.TRAIN.STEPSIZE)
                sess.run(tf.assign(lr, lr.eval() * cfg.TRAIN.GAMMA**(tiems)))
                print('current learning rate is:', lr.eval())
            ...
            res_fetches = []
            fetch_list = [
                total_loss, model_loss, rpn_cross_entropy, rpn_loss_box, cross_entropy, loss_box,
                summary_op, train_op
            ] + res_fetches

            total_loss_val, model_loss_val, rpn_loss_cls_val, rpn_loss_box_val, \
                cross_entropy_val, loss_box_val, summary_str, _ = sess.run(
                    fetches=fetch_list, feed_dict=feed_dict)

            # fetch_list_test = [
            #     rpn_label, rpn_cls_score, train_op] + res_fetches
            # rpn_label_val, rpn_cls_score_val, _ = sess.run(
            #     fetches=fetch_list_test, feed_dict=feed_dict)

            # print('rpn_label_val', rpn_label_val, np.shape(rpn_label_val))
            # print('rpn_cls_score_val', rpn_cls_score_val, np.shape(rpn_cls_score_val))
            ...
            if (iter) % (cfg.TRAIN.DISPLAY) == 0:
                print(
                    'iter: %d / %d, total loss: %.4f, model loss: %.4f, rpn_loss_cls: %.4f, rpn_loss_box: %.4f, cross_entropy: %.4f, loss_box: %.4f, lr: %f' %
                    (iter, max_iters, total_loss_val, model_loss_val, rpn_loss_cls_val, rpn_loss_box_val, cross_entropy_val, loss_box_val, lr.eval()))
