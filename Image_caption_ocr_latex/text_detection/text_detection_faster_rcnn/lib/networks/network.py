...
class Network(object):
    ...
    def make_var(self, name, shape, initializer=None, trainable=True, regularizer=None):
        return tf.get_variable(
            name, shape, initializer=initializer, trainable=trainable, regularizer=regularizer)
    ...
    def Bilstm(self, input, d_i, d_h, d_o, name, trainable=True):
        ...
        with tf.variable_scope(name) as scope:
            ...
            # 对输入进行reshape，NHWC转成 N*H,W,C
            img = tf.reshape(img, [N * H, W, C])
            img.set_shape([None, None, d_i])

            # 单层双向动态RNN，单向输入的隐藏层数为d_h，则双向的输出对应为(bacth,2*d_h)
            ...
            # 将正向和反向的输出进项contact,维度变成(N * H * W, 2 * d_h)
    ...
    @layer
    def MutBigru(self, input, d_i, d_h, d_o, name, num_step=3, trainable=True):
        img = input
        with tf.variable_scope(name) as scope:
            shape = tf.shape(img)
            N, H, W, C = shape[0], shape[1], shape[2], shape[3]
            ...
            for _ in range(num_step):
                stacked_rnn.append(tf.contrib.rnn.LSTMCell(d_h))
                stacked_bw_rnn.append(tf.contrib.rnn.LSTMCell(d_h))

            mcell = tf.contrib.rnn.MultiRNNCell(stacked_rnn)
            mcell_bw = tf.contrib.rnn.MultiRNNCell(stacked_bw_rnn)
            # 多层动态双向RNN，output中是一个长度为t的list，t和输入维度相同
            outputs, _, _ = tf.contrib.rnn.stack_bidirectional_dynamic_rnn(
                [mcell], [mcell_bw], img, dtype=tf.float32)
            ...
            weights = self.make_var('weights', [2 * d_h, d_o],
    ...
    def lstm(self, input, d_i, d_h, d_o, name, trainable=True):
        ...
        with tf.variable_scope(name) as scope:
            ...
            lstm_out, last_state = tf.nn.dynamic_rnn(
                lstm_cell, img, initial_state=initial_state, dtype=tf.float32)

            lstm_out = tf.reshape(lstm_out, [N * H * W, d_h])

            init_weights = tf.truncated_normal_initializer(stddev=0.1)
            init_biases = tf.constant_initializer(0.0)
            weights = self.make_var('weights', [d_h, d_o], init_weights,
                                    trainable, regularizer=self.l2_regularizer(cfg.TRAIN.WEIGHT_DECAY))
            biases = self.make_var('biases', [d_o], init_biases, trainable)
            outputs = tf.matmul(lstm_out, weights) + biases
    ...
    def lstm_fc(self, input, d_i, d_o, name, trainable=True):
        with tf.variable_scope(name) as scope:
            ...
            init_biases = tf.constant_initializer(0.0)
            kernel = self.make_var('weights', [d_i, d_o],
                                   init_weights, trainable,
                                   regularizer=self.l2_regularizer(cfg.TRAIN.WEIGHT_DECAY))
            biases = self.make_var('biases', [d_o], init_biases, trainable)
    ...
    def conv(self, input, k_h, k_w, c_o, s_h, s_w, name, biased=True, relu=True,
        ...
        :param k_h:  height of the kernel
        :param k_w:  width of the kernel
        :param c_o:  nums of the kernel
        :param s_h:  stride in the height direction
        :param s_w:  stride in the width direction
        :param name: name of the layer
        :param biased: 
        :param relu: activate function
        :param padding: padding or not
        :param trainable: trainable or not
        :return: 
        '''
        ...
        def convolve(inputs, kernels):
            return tf.nn.conv2d(inputs, kernels, [1, s_h, s_w, 1], padding=padding)

        with tf.variable_scope(name) as scope:
            init_weights = tf.truncated_normal_initializer(0.0, stddev=0.01)
            init_biases = tf.constant_initializer(0.0)
            kernel = self.make_var(
                'weights', [k_h, k_w, c_i, c_o],
                init_weights, trainable, regularizer=self.l2_regularizer(cfg.TRAIN.WEIGHT_DECAY))
            if biased:
    ...
    def proposal_layer(self, input, _feat_stride, anchor_scales, cfg_key, name):
        ...
        with tf.variable_scope(name) as scope:
            blob, bbox_delta = tf.py_func(
                proposal_layer_py, [input[0],
                                    input[1],
                                    input[2],
                                    cfg_key, _feat_stride, anchor_scales],
                [tf.float32, tf.float32])
            rpn_rois = tf.convert_to_tensor(
                tf.reshape(blob, [-1, 5]), name='rpn_rois')  # shape is (1 x H x W x A, 2)
    ...
    def anchor_target_layer(self, input, _feat_stride, anchor_scales, name):
        ...
        with tf.variable_scope(name) as scope:
            ...
            rpn_bbox_inside_weights = tf.convert_to_tensor(
                rpn_bbox_inside_weights, name='rpn_bbox_inside_weights')  # shape is (1 x H x W x A, 4)
            rpn_bbox_outside_weights = tf.convert_to_tensor(
                rpn_bbox_outside_weights, name='rpn_bbox_outside_weights')  # shape is (1 x H x W x A, 4)
    ...
    def reshape_layer(self, input, d, name):
        ...
        if name == 'rpn_cls_prob_reshape':
            ...
            # transpose: -> (1, H, W, 2xA)
            return tf.transpose(
                tf.reshape(tf.transpose(input, [0, 3, 1, 2]), [input_shape[0], int(d), tf.cast(
            ...
            return tf.transpose(
                tf.reshape(tf.transpose(input, [0, 3, 1, 2]), [input_shape[0], int(d), tf.cast(
                    ...
                    (tf.cast(input_shape[3], tf.float32) / tf.cast(
    ...
    def roi_pool(self, input, pooled_height, pooled_width, spatial_scale, name):
        ...
        print(input)
        return roi_pool_op.roi_pool(
            input[0],
            input[1],
            pooled_height, pooled_width, spatial_scale, name=name)[0]
        ...
        with tf.variable_scope(name) as scope:
            ...
            if input_shape.ndims == 4:
                ...
                for d in input_shape[1:].as_list():
                    dim *= d
                feed_in = tf.reshape(
                    tf.transpose(input, [0, 3, 1, 2]), [-1, dim])
            else:
    ...
    def batch_normalization(self, input, name, relu=True, is_training=False):
        """contribution by miraclebiu"""
        if relu:
            temp_layer = tf.contrib.layers.batch_norm(input,
                                                      scale=True,
                                                      center=True,
    ...
    def l2_regularizer(self, weight_decay=0.0005, scope=None):
        def regularizer(tensor):
            with tf.name_scope(
                    scope, default_name='l2_regularizer', values=[tensor]):
                l2_weight = tf.convert_to_tensor(
                    weight_decay, dtype=tensor.dtype.base_dtype, name='weight_decay')
                # return tf.mul(l2_weight, tf.nn.l2_loss(tensor), name='value')
                return tf.multiply(
                    l2_weight, tf.nn.l2_loss(tensor), name='value')
    ...
    def _modified_smooth_l1(
            self, sigma, bbox_pred, bbox_targets, bbox_inside_weights, bbox_outside_weights):
        """
        ...
        smooth_l1_result = tf.add(tf.multiply(smooth_l1_option1, smooth_l1_sign), tf.multiply(
            smooth_l1_option2, tf.abs(tf.subtract(smooth_l1_sign, 1.0))))
    ...
    def build_loss(self, ohem=False):
        ...
        rpn_cls_score = tf.reshape(
            self.get_output('rpn_cls_score_reshape'), [-1, 2])  # shape (HxWxA, 2)
        ...
        model_loss = rpn_cross_entropy + rpn_loss_box + cross_entropy + loss_box

        regularization_losses = tf.get_collection(
            tf.GraphKeys.REGULARIZATION_LOSSES)
        total_loss = tf.add_n(regularization_losses) + model_loss

        return total_loss, model_loss, rpn_cross_entropy, rpn_loss_box, cross_entropy, loss_box
