'''
File: anchor_target_layer_tf.py
Project: rpn_msr
File Created: Monday, 2nd April 2018 12:11:51 pm
Author: xiaofeng (sxf1052566766@163.com)
-----
Last Modified: Thursday, 21st June 2018 5:18:59 pm
...
def anchor_target_layer(
    ...
    # print('shift_Xx', shift_x)
    # print('shift_yy', shift_y)
    ...
    if DEBUG:
        ...
        print('over_laps', len(overlaps), len(overlaps[0]), np.shape(overlaps))

    # 存放每一个anchor和每一个gtbox之间的overlap
    # 在列的方向上查询最大值，并返回最大值对应的位置
    ...
    if DEBUG:
        print('label_max_overlaps', labels)
    ...
    # FIXME 此处对RPN_BATCHSIZE进行了修改-256--->512
    num_fg = int(cfg.TRAIN.RPN_FG_FRACTION * cfg.TRAIN.RPN_BATCHSIZE)
    fg_inds = np.where(labels == 1)[0]
    if DEBUG:
        print('fg_inds_nums', len(fg_inds))
    ...
    if DEBUG:
        print('shape:rpn_labels', np.shape(rpn_labels))
        print('shape:rpn_bbox_targets', np.shape(rpn_bbox_targets))
        print('shape:rpn_bbox_inside_weights', np.shape(rpn_bbox_inside_weights))
        print('shape:rpn_bbox_outside_weights', np.shape(rpn_bbox_outside_weights))
