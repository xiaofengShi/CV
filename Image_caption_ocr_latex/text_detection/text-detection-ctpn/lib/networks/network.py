...
class Network(object):
    ...
    def proposal_layer(self, input, _feat_stride, anchor_scales, cfg_key, name):
        ...
        with tf.variable_scope(name) as scope:
            ...
            rpn_rois = tf.convert_to_tensor(
                tf.reshape(blob, [-1, 5]),
                name='rpn_rois')  # shape is (1 x H x W x A, 2)
    ...
    def anchor_target_layer(self, input, _feat_stride, anchor_scales, name):
        ...
        with tf.variable_scope(name) as scope:
            ...
            rpn_labels = tf.convert_to_tensor(
                tf.cast(rpn_labels, tf.int32),
                name='rpn_labels')  # shape is (1 x H x W x A, 2)
            rpn_bbox_targets = tf.convert_to_tensor(
                rpn_bbox_targets, name='rpn_bbox_targets')  # shape is (1 x H x W x A, 4)
            ...
            return tf.transpose(
                tf.reshape(
                    ...
                    [input_shape[0],
                     int(d),
                     tf.cast(
                         tf.cast(input_shape[1],
                                 tf.float32) / tf.cast(d, tf.float32) * tf.cast(
                             input_shape[3],
                             tf.float32),
                         tf.int32),
                     input_shape[2]]),
                ...
                tf.reshape(
                    ...
                    [input_shape[0],
                     ...
                     tf.cast(
                         tf.cast(input_shape[1],
                                 tf.float32) *
                         (tf.cast(input_shape[3],
                                  tf.float32) / tf.cast(d, tf.float32)),
                         tf.int32),
                     input_shape[2]]),
                [0, 2, 3, 1],
                name=name)
    ...
    def batch_normalization(self, input, name, relu=True, is_training=False):
        """contribution by miraclebiu"""
        if relu:
            temp_layer = tf.contrib.layers.batch_norm(
                input, scale=True, center=True, is_training=is_training, scope=name)
